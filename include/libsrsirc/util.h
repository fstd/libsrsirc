...
/* lsi_ut_pfx2nick - Extracts the nick part from a `nick[!user][@host]-style' prefix
...
/* lsi_ut_parse_hostspec - parse a "host specifier" (i.e. something "host:port"-ish)
...
/* lsi_ut_parse_pxspec - parse a "proxy specifier", which is a (no-ssl) "host spec"
 ...
 *             hostspec is a host specifier as described in lsi_ut_parse_hostspec
...
/* lsi_ut_mut_nick - Default "nick mutilation" function to generate new nicknames
...
/* lsi_ut_clonearr - deep-copy a tokarr (usually containing a tokenized irc message)
 * Params: `arr': pointer to the tokarr to clone
 * Returns pointer to the cloned result */
tokarr *lsi_ut_clonearr(tokarr *arr);

/* lsi_ut_freearr - free a tokarr (which presumably was obtained by lsi_ut_clonearr) */
void lsi_ut_freearr(tokarr *arr);
